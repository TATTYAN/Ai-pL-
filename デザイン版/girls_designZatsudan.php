...
////////↓ここを置換
$rs = mysql_db_query("b31_c467","select * from destweet_tbl order by tweet_time desc");
////////↑destweet_tblの名前を変える
    ...
    <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
      <div class="container-fluid">
        <div class="navbar-header">
          ...
          <a class="navbar-brand" href="tweet.php">Adobe.design:<?php echo$name; ?></a>
        </div>
      </div>
    </nav>
        ...
        <div class="col-md-3">
          ...
          <form action ="tweet_desinf.php" method="GET">
            <br>
            投稿する作品URLとコメントを入力しよう！(100字以内)<br> 
...
while(true){
  ...
  if($row == null){
    ...
    echo "
    <tr>
    <td>{$row['tweet_time']} ：  {$row['account_name']}</td>
    <td><a href='{$row['url']}'>{$row['title']}</a></td>
    <td>{$row['contents']}</td>
    <td>
    <form action='girls_desiine.php' method='GET'>
    ...
    ////////↓ここのファイル名を変える
    echo"<td><form action='girls_desiine.php' method='GET'>";
    ///////↑ girls_desiineを置換 (ctrl + option + f)
  ...
  ///////ここのファイル名を変える
  echo"<td><form action='girls_descom.php' method='GET'>";
  ////////girls_descom.phpを置換
    echo"<input type='hidden' name='tweet_id' value='".$row['tweet_id']."'>
    <input type='hidden' name='account_name' value='".$row['account_name']."'>
    <input type='hidden' name='account_name' value='".$row['account_name']."'>
